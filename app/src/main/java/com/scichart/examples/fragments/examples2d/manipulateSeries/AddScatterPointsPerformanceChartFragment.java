...
//
// AddScatterPointsPerformanceChartFragment.java is part of SCICHART®, High Performance Scientific Charts
...
public class AddScatterPointsPerformanceChartFragment extends ExampleBaseFragment<ExampleAddPointsPerformanceFragmentBinding>{
    ...
    private void onAppendPoints(int count) {
        ...
        final DoubleSeries randomSeries = DataManager.getInstance().getRandomDoubleSeries(count);
        final IXyDataSeries<Double, Double> dataSeries = new XyDataSeries<>(Double.class, Double.class);
        dataSeries.append(randomSeries.xValues, randomSeries.yValues);

        int color = ColorUtil.rgb(random.nextInt(255), random.nextInt(255), random.nextInt(255));
        surface.getRenderableSeries().add(
                sciChartBuilder.newScatterSeries()
