...
import com.scichart.examples.utils.scichartExtensions.modifier
...
class PinchZoomOnRealtimeChartFragment : ExampleSingleChartBaseFragment() {
    private val ds1 = XyDataSeries<Double, Double>().apply { seriesName = "Orange Series";  }
    private val ds2 = XyDataSeries<Double, Double>().apply { seriesName = "Blue Series";  }
    private val ds3 = XyDataSeries<Double, Double>().apply { seriesName = "Green Series"; }

    private val scheduledExecutorService = Executors.newSingleThreadScheduledExecutor()
    private lateinit var schedule: ScheduledFuture<*>

    private var xAxis: NumericAxis? = null
    private var yAxis: NumericAxis? = null
    private val seriesValueModifier = SeriesValueModifier()
    override fun initExample(surface: SciChartSurface) {
        ...
        surface.suspendUpdates {
            Collections.addAll(xAxes, xAxis)
            Collections.addAll(yAxes, yAxis)
            ...
            if (binding.surface.gestureState == GestureState.INTERACTED) {
                UpdateSuspender.using(binding.surface) {
                    if (xAxis?.getAutoRange() != AutoRange.Never) {
                        ...
                        for (i in binding.surface.chartModifiers.indices) {
                            if (binding.surface.chartModifiers[i] is SeriesValueModifier) {
                                binding.surface.chartModifiers.removeAt(i)
                                break
                            }
                        }
                    }
                }
            } else if (binding.surface.gestureState == GestureState.NOT_INTERACTED) {
                UpdateSuspender.using(binding.surface) {
                    if (xAxis?.getAutoRange() != AutoRange.Always) {
                        xAxis?.setAutoRange(AutoRange.Always)
                        yAxis?.setAutoRange(AutoRange.Always)
                        Collections.addAll(
                            binding.surface.chartModifiers,
                            seriesValueModifier
                        )
                    }
                }
