...
//
// AddScatterPointsPerformanceChartFragment.kt is part of SCICHART®, High Performance Scientific Charts
...
class AddScatterPointsPerformanceChartFragment: ExampleBaseFragment<ExampleAddPointsPerformanceFragmentBinding>() {
    ...
    private fun onAppendPoints(count: Int) {
        val randomSeries = DataManager.getInstance().getRandomDoubleSeries(count)

        val color = ColorUtil.rgb(random.nextInt(255), random.nextInt(255), random.nextInt(255))

        binding.surface.suspendUpdates {
            renderableSeries {
                xyScatterRenderableSeries {
                    xyDataSeries<Double, Double> {
                        append(randomSeries.xValues, randomSeries.yValues)
                    }
                    strokeStyle = SolidPenStyle(color)
                    pointMarker = sciChartBuilder.newPointMarker(EllipsePointMarker())
                        .withSize(6, 6)
                        .withStroke(0xFFFFFFFF.toInt(), 0.1f)
                        .withFill(color)
                        .build()
                }
            }
