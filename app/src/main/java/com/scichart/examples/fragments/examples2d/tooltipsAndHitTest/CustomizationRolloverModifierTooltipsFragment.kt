...
//
// CustomizationRolloverModifierTooltipsFragment.kt is part of SCICHART®, High Performance Scientific Charts
...
class CustomizationRolloverModifierTooltipsFragment : ExampleSingleChartBaseFragment() {
    ...
    override fun initExample(surface: SciChartSurface) {
        ...
        surface.suspendUpdates {
            xAxes { numericAxis() }
            yAxes { numericAxis { axisInfoProvider = CustomAxisInfoProvider() } }
    ...
    private class SecondCustomSeriesInfoProvider : DefaultXySeriesInfoProvider() {
        override fun getSeriesTooltipInternal(context: Context, seriesInfo: XySeriesInfo<*>?, modifierType: Class<*>): ISeriesTooltip {
            return when (modifierType) {
                RolloverModifier::class.java -> { SecondCustomXySeriesTooltip(context, seriesInfo) }
                else -> { super.getSeriesTooltipInternal(context, seriesInfo, modifierType) }
            }
        }
    ...
    private class CustomAxisInfoProvider : DefaultAxisInfoProvider() {
        ...
        private class CustomAxisTooltip(context: Context?, axisInfo: AxisInfo?) : AxisTooltip(context, axisInfo) {
    ...
    companion object {
        ...
        private const val MODIFIER_NAME = "RolloverModifier"
    }
}